...
var ConsoleStatusLogger_1 = require("./ConsoleStatusLogger");
// Private configuration vars
var _appInsights;
var _logger = console;
var _prefix = "ad_"; // App Services, Default
var _statusLogger = new ConsoleStatusLogger_1.ConsoleStatusLogger();
// Env var local constants
var ENV_extensionVersion = "ApplicationInsightsAgent_EXTENSION_VERSION";
...
function setupAndStart(setupString) {
    ...
    if (!_extensionEnabled) {
        _statusLogger.writeFile(__assign({}, defaultStatus, { AgentInitializedSuccessfully: false, Reason: "Extension is not enabled. env." + ENV_extensionVersion + "=" + process.env[ENV_extensionVersion] }));
        return null;
    }
    // If app already contains SDK, skip agent attach
    if (Helpers.sdkAlreadyExists(_logger)) {
        _statusLogger.writeFile(__assign({}, defaultStatus, { AgentInitializedSuccessfully: false, SDKPresent: true, Reason: "SDK already exists" }));
        return null;
    }
    if (!setupString) {
        ...
        _statusLogger.writeFile(__assign({}, defaultStatus, { AgentInitializedSuccessfully: false, Reason: message }));
        return null;
    }
    try {
        ...
        _statusLogger.writeFile(__assign({}, defaultStatus, { AgentInitializedSuccessfully: true }));
    }
    catch (e) {
        _logger.error("Error setting up Application Insights", e);
        _statusLogger.writeFile(__assign({}, defaultStatus, { AgentInitializedSuccessfully: false, Reason: "Error setting up Application Insights: " + (e && e.message) }));
    }
    return _appInsights;
}
